...
class DFF_net(nn.Module):
    def __init__(self):
        ...
        self.pooling = nn.AvgPool3d((1,2, 2), stride=(1,2,2))
    ...
    def forward(self,FS,focus_dists):
        ...
        FR_volume=self.FR_module(quad_volume)
        FR_volume = self.pooling(FR_volume)
        RE_volume=FS_volume+FR_volume
        FS_mid_out = self.confidence(RE_volume)#1/8 size
        ...
        # AiF_attention = F.softmax(cost3, -3)
        # AiF = AiF_attention.repeat((1, 3, 1, 1, 1)) * FS[:, :3]
...
class Fourier(nn.Module):
    ...
    def forward(self, x):
        ...
        fm = fm.view(a, fm.shape[0] // a, *fm.shape[1:])
        fm = fm.transpose(1, 2)
        return fm